...
| FILE-001 | P | Files module (Drive folder links per scope + browse/attach) + summary + search | Drive domain | ✓ (summary by scope Total/Clients/Projects/Campaigns, search, full-width, states; link/browse/attach/unlink; nav «الملفات») | tsc+build; browser | **IMPLEMENTED_NOT_VERIFIED** | 2d003ae | unified /files library DONE (request_files+report_exports tenant-wide, source/client attribution, verified downloads, drive count); QA mobile+LTR+dark ok; remaining: preview pane, Firefox/WebKit |
| CLIENT-UI-SUMMARY | — | Clients portfolio top summary KPIs | clients domain | ✓ Total(server)/Active/Needs-attention/Open-requests + full-width | tsc+build; browser | **IMPLEMENTED_NOT_VERIFIED** | 7cf115a | cross-device verify |
| REQUEST-UI-SUMMARY | — | Requests inbox top summary KPIs | requests domain | ✓ Total(server)/New/Under-review/Waiting-client + full-width | tsc+build; browser (no h-scroll) | **IMPLEMENTED_NOT_VERIFIED** | 5225658 | cross-device verify |
