...
| SUBSCRIPTION-UI-001 | R | Subscriptions page (/app/billing) to Campaigns/Integrations level: summary KPIs + filters + search + professional table/cards + detail drawer + states + related entities; keep current design/fonts | billing domain | PARTIAL | — | **NOT_STARTED** | — | overhaul to reference level |
