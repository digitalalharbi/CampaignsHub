...
**Still to come in §15:** the Creative Groups UI (§15.13 — the endpoints exist, no UI) and the E2E half of the acceptance tests. §15.6, §15.10, §15.11 and §15.12 are done — see CREATIVE-014 … CREATIVE-039.

**Carried, not fixed here — the operator dashboard does not render its own findings.** `GET /creatives/pulse` has returned `insights` since `bbcddce`, and `CreativePulseSection` never draws them; §15.6 gives them a home on the detail page, but the dashboard section is still an API without a UI, which the contract forbids. It is a small addition to that one component and belongs with §15.13.
