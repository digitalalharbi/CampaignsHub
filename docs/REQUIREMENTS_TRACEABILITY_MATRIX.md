...
**Open, unreproduced:** one full-suite run showed two `DemoClientPortalTest` cases failing (the counters case and the branding case); the same suite passed on an immediate re-run, and the three classes together passed twice more in isolation. Recorded rather than closed — a re-run is not a fix, and this needs a root cause before the final gate.
