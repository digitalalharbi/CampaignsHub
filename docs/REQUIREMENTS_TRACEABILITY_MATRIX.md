...
**Still to come in §15:** the E2E half of the acceptance tests (Light/Dark, Tablet, three browsers, PDF export ceilings). §15.6, §15.8, §15.10, §15.11, §15.12 and §15.13 are done — see CREATIVE-014 … CREATIVE-046.
...
**Stale demo rows in the development database, not a code defect.** 2,040 `creative_daily_metrics` rows carry fractional clicks and conversions — every one written 2026-08-05 22:09, before the seeder fix at `93897cc` (2026-08-06 19:07). The guard test re-seeds and passes, which is the proof the seeders no longer produce them; the dev database simply still holds the old rows, so a client link opened against it shows «178.2 orders». Re-seeding the demo data clears it.
