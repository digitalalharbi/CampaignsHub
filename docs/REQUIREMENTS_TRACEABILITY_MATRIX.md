...
| ID | AREA | Requirement | Backend | Frontend | Tests | Status | Commit | Notes |
|---|---|---|---|---|---|---|---|---|
| REPORT-FORM-001 | REPORTS | «Executive summary» and «detailed» are a report's own axis, independent of live/snapshot | `reports.form` (migration `2026_08_09_120000`, backfilled from `type='executive'` or `audience='executive'`, defaulting to `detailed`); validated in `ReportController` | The builder offers the two forms as a labelled pair with what each contains | `ReportFormTest` (8 cases, 37 assertions) | **VERIFIED** | `01e6ce2` | Three questions were sharing two columns: `type` (what a report is ABOUT), `mode` (whether a link recomputes) and — smuggled into both — «summary or full detail». `type = 'executive'` sat in a list beside `campaign` and `weekly`, so a monthly report could not be issued as a summary without claiming to be a different kind of report |
| REPORT-FORM-002 | REPORTS | A shared link honours the form | `PublicReportController::show` applies `ClientReportView::executive()` for a summary and `filter()` otherwise; `live` reports the form too | The client page names which document it is, in both languages | `…::the_summary_link_is_trimmed_and_the_detailed_link_is_not` | **VERIFIED** | `01e6ce2` | **Real defect.** Every shared link ran the plain client filter, so a report an operator had deliberately built as an executive summary arrived in full detail. The setting was honoured in the PDF export and dropped on the link — the copy most clients actually open. Live: `/r/pYFa…` renders 4 slides, `/r/pLTO…` renders 6, both with no session |
| REPORT-FORM-003 | REPORTS | Either form can be live or a snapshot — all four combinations | — | — | `…::either_form_can_be_live_or_a_snapshot` | **VERIFIED** | `01e6ce2` | A link is live only when given a scope to stay inside (LIVEREP-001). That is a fact about delivery and says nothing about whether the document is five pages or thirty |
| REPORT-LINK-URL-001 | REPORTS | The shareable link is the canonical `https://campaignshub.io/r/<token>` | `ShareService::urlFor()` from `brand.frontend_url`; three controllers return `url` (absolute) and `path` (relative) | The copy button uses the server's URL | covered by the share tests | **VERIFIED** | `01e6ce2` | The page built it from `window.location.origin` — right on production and quietly wrong everywhere else. An operator reviewing on staging, a preview deployment or localhost copied a host only they could reach and sent it to a client: silent on the sending side, total on the receiving one |
