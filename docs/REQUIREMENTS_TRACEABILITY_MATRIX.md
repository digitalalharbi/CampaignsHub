...
| ID | AREA | Requirement | Backend | Frontend | Tests | Status | Commit | Notes |
|---|---|---|---|---|---|---|---|---|
| IDENTITY-PROD-001 | ADMIN | The platform's published address is `info@campaignshub.io`, spelled one way | Migration `2026_08_09_090000` changes the column default and rewrites rows still holding the old default; `PlatformSetting::current()` states the same value | Marketing copy, `legalTypes.CONTACT_EMAIL`, the `/admin` legal editor | `PlatformIdentityTest` (5) + `PlatformLegalSettingsTest` (12) | **VERIFIED** | `3f52d20` | It shipped as `info@CampaignsHub.io`. A mail domain is case-insensitive so nothing was undeliverable, but this address is PRINTED on the privacy policy, the terms, the contact page and the security page, and compared as a string in code. Mixed case in a published address reads as a typo. Live: `/contact` and `/data-requests` both render `info@campaignshub.io` and nothing else |
| IDENTITY-PROD-002 | ADMIN | The support address is an address somebody actually reads | `config/brand.php` `support_email` default → `info@campaignshub.io`; `.env.example` likewise | `lib/brand.ts` `supportEmail` | `PlatformIdentityTest::support_falls_to_the_one_address…` | **VERIFIED** | `3f52d20` | It defaulted to `support@campaignshub.io` — not a typo but a **second mailbox nobody holds**, printed on the contact page. Inviting a customer to write somewhere no one is listening is the same failure as claiming a delivery that did not happen |
| IDENTITY-PROD-003 | ADMIN | Outgoing mail comes FROM the platform | `MAIL_FROM_ADDRESS` → `info@campaignshub.io` in `.env.example` and `.env` | — | — | **VERIFIED** | `3f52d20` | It was `hello@example.com` — the Laravel skeleton default. Every invoice, receipt, report and verification mail the product ever sent would have carried it |
| IDENTITY-PROD-004 | ADMIN | Every brand URL falls back to the production domain | `config/brand.php` | — | `PlatformIdentityTest::every_brand_url_falls_back…` | **VERIFIED** | `3f52d20` | Asserted against the DEFAULTS written in the config file, not the resolved values: those are whatever the machine running the tests has in `.env` (here `localhost:5173`), which is right for a developer and proves nothing about a deployment |
| IDENTITY-PROD-005 | ADMIN | A deployment on campaignshub.io knows exactly what to set | `.env.example` gains a Production block: `APP_ENV/APP_DEBUG/APP_URL/FRONTEND_URL`, `SANCTUM_STATEFUL_DOMAINS`, `SESSION_DOMAIN=.campaignshub.io`, `SESSION_SECURE_COOKIE=true`, `SESSION_SAME_SITE=lax`, the two addresses | — | — | **VERIFIED** | `3f52d20` | Each is load-bearing and each fails quietly: a missing Sanctum origin returns 401 rather than erroring (reads as an authentication defect), and a `SESSION_DOMAIN` without the leading dot leaves a customer who signed in on one host arriving at the other as a guest |
