...
| AGENCY-006-B | AGENCY | The client picker stays usable as the roster grows | **VERIFIED** | `d290e76` | A filter box appears above the native select past 12 options; the select stays native so keyboard and screen-reader behaviour is unchanged in both directions, and the currently selected client is never filtered out (that would read as «nothing selected» while the pages below still show its data). An empty filter result says so rather than looking like «you have no clients». Found live: 269 `<option>`s in one scroll, and this control gates every project-scoped page. `AgencyScopeSwitcher.test.tsx` (4 cases); live 269 → 1 on typing `Acme` |
