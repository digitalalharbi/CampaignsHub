...
| ID | AREA | Requirement | Backend | Frontend | Tests | Status | Commit | Notes |
|---|---|---|---|---|---|---|---|---|
| REPORT-OBJECTIVE-003 | REPORTS | The generated report and its client link carry Direct and Blended separately | `objective_performance` in the snapshot, built from the same `daily_metrics` as the rest of it — not a second source | `ObjectiveSplitSlide` — both figures side by side, each with its formula printed, plus every path and every excluded campaign named with its spend | `ObjectivePerformanceTest::the_generated_report_and_its_client_link_keep_the_two_figures_apart` | **VERIFIED** | `52f4d9d` | Live on the demo store, real data: **Direct CPA 73.72 · Blended CPA 90.95**, with 16,839 SAR of non-sales spend named. Rendered through the client link with no session |
| REPORT-OBJECTIVE-004 | REPORTS | A report never prints a bare `CPA` when it means a blended one | — | The executive slide's cards and its exact-figures strip read «CPA (مبيعات مباشرة)» from `direct`; the blended pair is shown under its own name | live journey | **VERIFIED** | `52f4d9d` | **Critical defect.** The summary slide printed `CPA` and `ROAS` straight from `kpis`, which rolls every path together. On this demo project it read 81.82 — neither the direct 73.72 nor the blended 90.95, since `kpis.cpa` divides all spend by ALL conversions including the lead campaign's. Three numbers, one label |
| REPORT-OBJECTIVE-004-A | REPORTS | The split survives into the summary form | `ClientReportView::EXECUTIVE_SLIDE_TYPES` keeps `objective_performance` | — | `ReportFormTest` + the live journey | **VERIFIED** | `52f4d9d` | The summary is where a blended CPA does the most damage: it is the version that gets forwarded and quoted, with no per-platform pages behind it to argue with. Trimming the section that says which figure is which would leave the most-read document the least qualified |
| UNIFIED-002-B | ANALYTICS | One definition of «an order» across every surface | `ObjectivePerformance` counts `conversions` alone | — | as above | **VERIFIED** | `52f4d9d` | It counted `purchases + conversions`, which made it more complete and made it DISAGREE with the dashboard, the breakdowns and the report's own `kpis` — all of which divide by `conversions`. Two definitions means the report's CPA and the dashboard's differ for the same scope and nobody can say which is the product's answer. One definition applied everywhere beats a better one applied in a single place |
| QA-FUNNEL-SHAPE-001 | QA | The snapshot's `funnel` stays a list of stages | Unpacked at both call sites (`ReportGenerator`, `LiveReportService`) | — | `ReportsTest::the_snapshot_funnel_stays_a_list_of_stages` | **VERIFIED** | `52f4d9d` | **Regression I introduced at `a08a6b2`.** Making the aggregator return `['stages','spend']` silently emptied the funnel slide of every report — `InteractiveReport` maps over `data.funnel`, and an object is not an array. The entire backend suite passed with it broken, because nothing asserted the shape of a key whose only consumer is a React component |
