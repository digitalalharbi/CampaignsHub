...
**Still to come in §15:** the executive and detailed report creative sections (§15.12), share-level creative permissions (fail-closed), recommendations (§15.10), the interactive Creative Details PAGE (§15.6 — the API is live and the viewer is built; the drill-down currently ends at the viewer), the Creative Groups UI (§15.13 — the endpoints exist, no UI), and the E2E half of the 24 acceptance tests.
...
**Seen while verifying, not fixed here:** the demo seeder writes fractional `conversions`, so an aggregate reads «Orders 1,288.75». It is demo data rather than product behaviour, and correcting it belongs with the seeder rather than in a dashboard commit.
