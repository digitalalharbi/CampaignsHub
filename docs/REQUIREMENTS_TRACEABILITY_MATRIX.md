...
| ID | AREA | Requirement | Backend | Frontend | Tests | Status | Commit | Notes |
|---|---|---|---|---|---|---|---|---|
| PORTALS-SWEEP-001 | UIQA | Every page of `/app` renders its own content under the advertiser account | — | — | live walk of all 13 rail destinations | **VERIFIED** | `6a5a5ad` | dashboard · projects · campaigns · content · analytics · reports · alerts · tasks · files · connections (→ integrations) · subscriptions · settings. No console error, no failure panel |
| PORTALS-SWEEP-002 | UIQA | Every page of `/portal` renders under the client account | — | — | live walk of all 9 destinations | **VERIFIED** | `6a5a5ad` | requests · quotes · invoices · campaigns · reports · files · messages · profile · home |
| PORTALS-SWEEP-003 | UIQA | The advertiser portal's `team` section has an address that resolves | — | `/app/team` → `/app/settings/permissions` | live | **VERIFIED** | `6a5a5ad` | `Portal::App::sections()` offers `team` and the server sends it in `account.nav`, but the advertiser's team lives inside Settings (deliberately — an agency scopes people to clients, an advertiser has one workspace), so the address answered 404 while the identical `/agency/team` worked. A moved section redirects: ADR 0002 decision 5 |
| AGENCY-PERMS-006 | PERM | The failure classification reaches every surface a signed-in user lands on | — | `ErrorState` takes an optional `error` and delegates to `QueryFailure`; the whole `/portal` tree wired directly | `States.test.tsx` (5 cases) + 668 existing | **VERIFIED** | `6a5a5ad` | Was PARTIAL at `23d64c2`. Fixed at the primitive rather than at ~30 call sites: `ErrorState` is what those screens already render, so one prop per call site replaced a rewrite of each. It also read «Retry» in English on every Arabic page in the product; it now takes the language from the interface when the caller does not say |
