...
| FUNNEL-NULL-001 | METRICS | A funnel stage no platform reported is `null`, not a measured 0 | — | — | — | **NOT_STARTED** | — | Found live at `7c115d4`: the demo funnel answers `landing_page_views=0 · add_to_cart=0 · checkout=0` from `COALESCE(SUM(…), 0)` over stages nobody has ever sent. Harmless while the pipeline could not carry those stages; a defect now that `PIPELINE-METRICS-001` can. «0 add to cart» beside 176 purchases reads as a broken funnel to a client. Needs `null` for no-rows distinct from `0` for rows-summing-to-zero, through `funnel()`, `CampaignFunnelTab`, `FunnelTab`, `CreativeFunnel` and the client report — the pattern `CreativeMetrics.reported` already set |
...
**Still part 2:** §14.5 scope customisation (platforms, accounts, ad sets, ads, creatives, objectives, paths, visible metrics, saved templates), §14.6 objective-aware layouts, §14.7–14.8 comparisons and recommendations. §14.9 attribution transparency (`REPORT-OBJECTIVE-005`) is **VERIFIED** — see the five rows above.
