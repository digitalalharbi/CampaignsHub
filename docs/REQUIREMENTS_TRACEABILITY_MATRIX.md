...
| HOME-012 | A HOME | Journey param VALUES corrected: `self-managed`→`self-service`, `agency`→`multi-client` (routes `/register?journey=self-service&module=paid-media` and `?journey=multi-client&module=paid-media`); RegisterPage + tests + homepage E2E must read the new values and preset accordingly | Sanctum | needs edit | — | **NOT_STARTED** | — | touch: homeCopy.ts routes, PublicHomePage tests, RegisterPage journey handling+tests, homepage.spec/registration-onboarding E2E |
| HOME-013 | A HOME | Differentiated public experiences per portal (paid-media / influencer-UGC / client-tracking) — distinct marketing copy + preview + persuasion, SAME design system; not one page with a changed title | n/a | — | — | **NOT_STARTED** | — | per-journey hero copy/preview variants driven by ?journey/?module |
| AUTH-002 | B AUTH | Login marketing panel adapts to the incoming portal/journey (paid-media vs client-tracking) | n/a | — | — | **NOT_STARTED** | — | read ?journey/?redirect on /login, swap the right-pane copy |
