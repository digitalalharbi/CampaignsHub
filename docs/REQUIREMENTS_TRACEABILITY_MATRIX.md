...
| REPORT-OBJECTIVE-005 | REPORTS | Attribution transparency and de-duplication | Orders are `purchases + conversions`, never summed with a third key that would double-count the same sale | — | — | **NOT_STARTED** | — | Only the double-count guard is in place. Model, window, click- vs view-through, platform-reported vs store-confirmed and dedup status are part 2 |
...
**Still part 2:** §14.5 scope customisation (platforms, accounts, ad sets, ads, creatives, objectives, paths, visible metrics, saved templates), §14.6 objective-aware layouts, §14.7–14.8 comparisons and recommendations, §14.9 attribution transparency (`REPORT-OBJECTIVE-005`, still `NOT_STARTED`).
