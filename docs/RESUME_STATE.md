...
- **EXACT NEXT REQUIREMENT = DASH-010-E** (compare-period + saved views with REAL persistence — a saved_views table/API keyed to user/tenant, not local state; + restore/rename/delete/default + compare-to-previous-period). DASH-010-D DONE (backend metric expansion + objective filter + KPI switching). Then DASH-010-F (remaining filter dimensions client/project/campaign/status/ad-account/region, backend-supported) → DASH-010-G (full dashboard verification incl live browser + isolation) → CAMPAIGN → CAMPDET → CREATIVE → ALERT → MESSAGE → FINANCE → TASK → FILE → PROJECT-INTEGRATION → per-platform → NORM/REPORT/REQUEST → DEMO → cross-module → visual → regression → clean-install → handover.
