...
## ⏳ IN FLIGHT — v4 homepage-audience + auth-nav correction (agent `a32f3d515ef21ef49`, off `af1cccb`)
Spec §"⚑⚑⚑ v4". Public homepage = external users (SaaS + service clients), NOT admins. Removing «تسجيل دخول النظام» wording; header actions (ابدأ الآن/تسجيل الدخول/اطلب خدمة/دخول العميل, auth→لوحة التحكم); journey buttons+routes (self-managed/agency add `&module=paid-media`, #4→`/requests/new?module=influencer-marketing`); below-card «دخول مساحة العمل»/«متابعة طلباتي»; business-model copy + 2 commercial tracks; intake accepts `?module=influencer-marketing`; hidden `/operations/login` alias + `public/robots.txt` noindex. After it lands: reconcile → build/vitest → browser-verify → commit.

## ⏳ REMAINING after the correction (finish the phase — no interim reports)
T10 Forms UX overhaul (steppers/draft/validation/error-summary/searchable/create/dependent/review across Register/Onboarding/Requests/Clients/Projects/Campaigns/Reports/Integrations/Alerts/Billing/Settings). Then T13/T14 full E2E regression (Chromium/Firefox/WebKit + RTL/LTR + light/dark + mobile; three apps; 0 failed/0 flaky/0 skipped-non-external). Only THEN send the final report.
