...
- **EXACT NEXT REQUIREMENT = HOME-012** (new user correction, precedence): rename journey values `self-managed`→`self-service`, `agency`→`multi-client` across homeCopy.ts routes + RegisterPage handling + PublicHomePage/registration E2E + tests; verify preset still works end-to-end. Then HOME-013 (per-portal differentiated public experiences) + AUTH-002 (adaptive login panel). Then resume DASH-010-D (objective KPIs) → E/F/G → CAMPAIGN → CREATIVE → ALERT → MESSAGE → FINANCE → TASK → FILE → PROJECT-INTEGRATION → … per matrix. Done this session: UNIFIED-001/002 (2a2d4b5), HOME-010 (34aa1e1), HOME-011 compact hero (4e0e2d0), DASH-010-A backend platform filter (6c1d373, MetricsTest 16), DASH-010-B/C dashboard filter bar (30ee70f, 209 vitest+9 E2E).
