...
- **EXACT NEXT REQUIREMENT = DASH-010** (dashboard command center). HOME-010 (34aa1e1) + HOME-011 compact hero (4e0e2d0) VERIFIED. DASH-010 plan (backend-supported, not React-only): (1) MetricsAggregator add `forProviders(array)` clone + `whereIn(daily_metrics.provider,...)` in base(); (2) MetricsController read `provider`+`objective` query params and scope every method via a `scoped($request)` helper (summary/timeseries/platforms/campaigns/funnel/budget + freshness direct query); (3) frontend analytics/api.ts+hooks accept a filters object (provider[]/objective/status/...) in the query string+queryKey; (4) DashboardPage filter bar (period+platform chips+objective select+reset+saved-view) feeding all tiles via UnifiedCampaignOverview; (5) objective-specific KPI set; (6) backend test `?provider=meta` filters totals/platforms; commit per slice. See REQUIREMENTS_TRACEABILITY_MATRIX.md.
