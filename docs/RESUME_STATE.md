...
- **EXACT NEXT REQUIREMENT = HOME-013** (per-portal differentiated public experiences: paid-media / influencer-UGC / client-tracking — distinct hero copy+preview+persuasion driven by ?journey/?module, same design system). Then AUTH-002 (adaptive /login marketing panel), DASH-010-D (objective-specific KPIs) → E/F/G, then CAMPAIGN → CAMPDET → CREATIVE → ALERT → MESSAGE → FINANCE → TASK → FILE → PROJECT-INTEGRATION → per-platform integrations → NORM/REPORT/REQUEST → DEMO → cross-module → visual → regression → clean-install → handover (matrix has all). Verified this session: UNIFIED-001/002 (2a2d4b5), HOME-010 (34aa1e1), HOME-011 (4e0e2d0), DASH-010-A (6c1d373), DASH-010-B/C (30ee70f), HOME-012 (a9157bc).
