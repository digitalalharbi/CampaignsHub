...
- **EXACT NEXT REQUIREMENT = DASH-010-D** (objective-specific KPI sets — awareness/traffic/leads/sales/app/engagement; needs an objective filter added to the backend filter contract like DASH-010-A did for provider, then UnifiedCampaignOverview KPI list switches by selected objective; mixed objectives→shared metrics, never ROAS-for-all). Then DASH-010-E (compare-period/saved-views) → F (remaining filter dimensions client/project/campaign/status/ad-account/region, backend-supported) → G (live browser click-through + dimension tests) → CAMPAIGN → CAMPDET → CREATIVE → ALERT → MESSAGE → FINANCE → TASK → FILE → PROJECT-INTEGRATION → per-platform → NORM/REPORT/REQUEST → DEMO → cross-module → visual → regression → clean-install → handover (full list in matrix). Verified this session: UNIFIED-001/002, HOME-010, HOME-011, DASH-010-A, DASH-010-B/C, HOME-012, HOME-013, AUTH-002.
