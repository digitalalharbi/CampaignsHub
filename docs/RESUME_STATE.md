...
- **EXACT NEXT REQUIREMENT = DASH-010-F** (thread ALL filter dimensions through every dashboard section via one filter context: objective+provider done; add client/project/campaign/status/ad-account/region backend filters + compare-period toggle UI using summary delta). Done: objective-integrity fix (9c1c92e: default=Awareness, no blended mixed KPIs, objective-titled page), DASH-010-E backend (faed0c9), DASH-010-E-FE saved-views UI. Then DASH-010-G (full cross-browser/mobile/RTL-LTR/light-dark + isolation + saved-views/compare/filters-synchronized verification → DASH-010 VERIFIED) → CAMPAIGN → CAMPDET → CREATIVE → ALERT → MESSAGE → FINANCE → TASK → FILE → PROJECT-INTEGRATION → per-platform → NORM/REPORT/REQUEST → DEMO → cross-module → typography-system-wide → visual → regression → clean-install → handover. Apply the no-mixed-objective rule to Analytics/Reports/Alerts/Creatives/marketing preview too.
