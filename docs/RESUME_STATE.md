...
- **EXACT NEXT REQUIREMENT = DASH-010-D** (objective-specific KPI sets — awareness/traffic/leads/sales/app/engagement; UnifiedCampaignOverview KPI list switches by selected objective; mixed→shared metrics, never ROAS-for-all). Done: DASH-010-A backend platform filter (6c1d373, MetricsTest 16), DASH-010-B/C frontend filter bar (30ee70f, 209 vitest + 9 E2E). Then E compare-period/saved-views → F remaining filter dimensions (client/project/campaign/status/ad-account/region, backend-supported) → G live browser click-through + dimension tests. HOME-011 VERIFIED (4e0e2d0).
