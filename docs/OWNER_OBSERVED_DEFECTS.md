...
| 14 | P0 | Large numbers must use canonical compact notation | «1,284,663 SAR» beside a card reading «1.28M SAR» | /app/analytics; entity tables | NUMBER-PRESENTATION-001 | NUMBER-PRESENTATION-001=PARTIAL | Family summary fallbacks called `toLocaleString` directly | yes (#298) | no | no | no | Deploy then Production check | — |
...
| 41 | P1 | Filters persist through refresh/back/deep-link | The library forgot the chosen view | /app/content; analytics | ANALYTICS-FILTER-TRUTH-001 | ANALYTICS-FILTER-TRUTH-001=IN_PROGRESS | `view` was state with no seed and no write-back; now the address is the only source | yes | no | no | no | Deploy then owner check | — |
