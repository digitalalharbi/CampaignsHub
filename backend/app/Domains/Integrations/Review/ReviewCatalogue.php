    ...
    private static function specific(): array
    {
        return [
            ...
            'x' => [
                ...
                self::item('oauth1a_user_authentication', 'declared', 'تفعيل OAuth 1.0a في إعدادات مصادقة المستخدم', 'OAuth 1.0a enabled in User authentication settings',
                    'واجهة إكس الإعلانية تقبل الطلبات الموقّعة بـ OAuth 1.0a فقط؛ تطبيق مُعدّ بـ OAuth 2.0 وحده لا يصل إلى أي حساب إعلاني.',
                    'The X Ads API accepts only OAuth 1.0a signed requests; an app set up for OAuth 2.0 alone reaches no ad account.'),
                ...
                self::item('access_token_regenerated', 'declared', 'إعادة توليد Access Token بعد الاعتماد', 'Access Token regenerated after approval',
                    'الرمز يحمل صلاحيات التطبيق وقت توليده؛ رمز وُلِّد قبل اعتماد Ads API لا يثبت شيئًا في اختبار الإعداد.',
                    'A token carries the app\'s permissions from the moment it was generated; one generated before Ads API approval proves nothing in the configuration test.'),
            ],
            'salla' => [
