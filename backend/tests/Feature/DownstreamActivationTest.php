...
final class DownstreamActivationTest extends TestCase
{
    ...
    /**
     * RUNTIME-100 §7 §21 — the rest of the structure, filed under the same project.
     *
     * Ad sets, ads and creatives are separate tables written by `ImportExternalStructure`, each from
     * the campaign's project. Asserting only on campaigns would leave three tables free to inherit a
     * project from somewhere else without anybody noticing.
     */
    public function test_ad_sets_ads_and_creatives_land_in_the_same_project(): void
    {
        $this->firstSync();

        foreach ([ExternalAdSet::class, ExternalAd::class, ExternalCreative::class] as $model) {
            $projects = $model::withoutGlobalScopes()->distinct()->pluck('project_id')->all();

            // A provider need not report every level; what it DID report must be scoped.
            if ($projects === []) {
                continue;
            }

            $this->assertSame([$this->projectB->id], $projects, $model.' escaped the assigned project');
        }
    }
