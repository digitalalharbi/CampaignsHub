...
describe('safeRedirect', () => {
  ...
  it('rejects open-redirect and off-site targets', () => {
    expect(safeRedirect('https://evil.example')).toBe('/app/dashboard')
    expect(safeRedirect('//evil.example')).toBe('/app/dashboard')
    expect(safeRedirect('/\\evil.example')).toBe('/app/dashboard')
    expect(safeRedirect('javascript:alert(1)')).toBe('/app/dashboard')
    expect(safeRedirect('relative/path')).toBe('/app/dashboard')
  })

  it('never bounces back to an auth page (no login loop)', () => {
    expect(safeRedirect('/login')).toBe('/app/dashboard')
    expect(safeRedirect('/login?redirect=/x')).toBe('/app/dashboard')
    expect(safeRedirect('/register')).toBe('/app/dashboard')
    expect(safeRedirect('/forgot-password')).toBe('/app/dashboard')
