...
export function CampaignsHubLogo({
  ...
  const body =
    variant === 'mark' ? (
      <CampaignsHubMark size={s.mark} className="text-brand-mark" title={name} />
    ) : (
      <span className={`inline-flex items-center gap-3 ${className}`}>
        <CampaignsHubMark size={s.mark} className="shrink-0 text-brand-mark" />
        <span className={`flex flex-col ${ar ? 'items-start text-start' : 'items-start text-start'}`}>
          <span className={`${s.title} font-heading font-extrabold leading-none tracking-tight`}>
            {/*
  ...
  if (!href) {
    return (
      <span className="inline-flex items-center" data-testid="campaignshub-logo">
        {body}
      </span>
    )
  }
