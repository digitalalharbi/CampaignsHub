...
for (const width of [320, 375, 390]) {
  test(`/login mobile ${width}px: single-column, no horizontal scroll, submit validates`, async ({ page }) => {
    ...
    const overflow = await page.evaluate(() => document.documentElement.scrollWidth > document.documentElement.clientWidth + 1)
    expect(overflow).toBe(false)
    // The card must be reachable at the top (the marketing panel is hidden below lg).
    await expect(page.getByTestId('login-identify')).toBeVisible()
    await expect(page.getByTestId('login-email')).toBeVisible()

    // An empty submit goes nowhere: both fields are required before the server is asked anything.
