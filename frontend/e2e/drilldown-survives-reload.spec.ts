...
test.describe('a drilled-in analytics link survives a reload', () => {
  ...
  test('comes back to the same rung, still narrowed to the same parent', async ({ page, request }) => {
    ...
    await page.goto('/agency/analytics?tab=ad_sets')
    ...
    // The address is the whole point of the clause: it has to carry the rung.
    ...
    // And what it shows: the same rung, named the same way.
